...
Setup
```{r}
model_statistics <- function(model, data) {
  # MSE of the model
...
#### Approach 0: Using Main Effect Model without box-cox transformation
...
#### Approach 1: Using Main Effect Model with box-cox transformation
...
Print model statistics
```{r}
model_statistics(full_main_model_box_cox, training_data)
```
...
#### Approach 3: Apply Stepwise AIC

Now, we can apply a stepwise AIC to main effect_model
```{r}
simple_model = lm(song_popularity_box_cox ~ 1, data=training_data)
stepwise_model = stepAIC(simple_model, scope = list(upper=full_main_model_box_cox, lower=simple_model),direction="both",trace=TRUE)
```

Therefore, we get this final model
```{r}
...
Note: Approach 2 and Approach 3 output the same model... so basically we did stepwise ourselves.
...
Fit a full interaction model with box cox transformation
```{r}
full_interaction_model_box_cox = lm(data = training_data, song_popularity_box_cox ~ song_duration_ms +acousticness+
                  danceability+ energy+ instrumentalness+ liveness+ loudness+ speechiness+
                  tempo+ audio_valence + as.factor(key)+ as.factor(audio_mode)+ as.factor(time_signature)+
                  acousticness:loudness+ audio_valence:danceability+ audio_valence:energy+ energy:loudness+
                  acousticness:energy+ energy:instrumentalness+ loudness:instrumentalness+
...
#### Approach 1: Full interaction model with box-cox transformation

This is the full interaction model
```{r}
summary(full_interaction_model_box_cox)
```

Print model statistics
```{r}
model_statistics(full_interaction_model_box_cox, training_data)
```

#### Approach 2: Drop predictors using F-test

```{r}
reduced_interaction_model = lm(song_popularity_box_cox ~ instrumentalness + audio_valence + as.factor(key) + as.factor(audio_mode) + as.factor(time_signature), data=training_data)
```

```{r}
anova(reduced_interaction_model, full_interaction_model_box_cox)
```

```{r}
f_stat = ((19980203 - 19093972) / 169) / (19093972 / 11751)
...
```{r}
simple_model_interaction = lm(song_popularity_box_cox ~ 1, data=training_data)
stepwise_interaction_model = stepAIC(simple_model, scope = list(upper=full_interaction_model_box_cox, lower=simple_model_interaction),direction="both",trace=TRUE)
```
...
```{r}
library(car)
vif(full_main_model_dropped_box_cox)
```
...
Print model statistics
```{r}
model_statistics(full_main_model_dropped_box_cox, dropped_training_data)
```
